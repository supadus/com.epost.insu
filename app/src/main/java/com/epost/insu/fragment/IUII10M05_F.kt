 ...
 * </pre>
 */
class IUII10M05_F : IUII10M00_FD() {
    private val maxInsureName: Int = 40 // 보험사명 맥스 length
    private val maxCountOtherInsure: Int = 4 // 타보험사 최대 수
    private val maxLengthCarInsure: Int = 40 // 자동차보험 보험사명 맥스 length
    ...
    private fun restoreData() {
        ...
        val tmp_arrOtherInsureMore: ArrayList<String> = data!!.CF_getArrOtherInsureMore()
        val tmp_arrRealInsureMore: ArrayList<String> = data!!.CF_getArrRealInsureMore()
        ...
        if (switchJoinOther!!.CF_getCheckState() > 0) {
            ...
            for (i in tmp_arrOtherInsureMore.indices) {
                createInputCompany(linOtherInsureMore, tmp_arrOtherInsureMore.get(i), object : View.OnClickListener {
                    override fun onClick(v: View) {
    ...
    private fun setUIOfFlagDo() {
        ...
        switchCarInsure!!.CF_getBtnLeft().contentDescription = resources.getString(R.string.desc_car_insure_yes_btn)
        switchCarInsure!!.CF_getBtnRight().contentDescription = resources.getString(R.string.desc_car_insure_no_btn)
        ...
        switchIndustryInsure?.CF_getBtnLeft()?.contentDescription = resources.getString(R.string.desc_industry_insure_yes_btn)
        switchIndustryInsure?.CF_getBtnRight()?.contentDescription = resources.getString(R.string.desc_industry_insure_no_btn)
        ...
        switchCallPolice?.CF_getBtnLeft()?.contentDescription = resources.getString(R.string.desc_police_yes_btn)
        switchCallPolice?.CF_getBtnRight()?.contentDescription = resources.getString(R.string.desc_police_no_btn)
    ...
    private fun setUIOfOtherInsureJoin() {
        ...
        val tmp_linInputOtherInsure: View = view!!.findViewById(R.id.linOtherInsureInput)
        edtOtherInsure = tmp_linInputOtherInsure.findViewById(R.id.editText)
        ...
        switchJoinOther?.CF_getBtnLeft()?.contentDescription = resources.getString(R.string.desc_other_join_yes_btn)
        switchJoinOther?.CF_getBtnRight()?.contentDescription = resources.getString(R.string.desc_other_join_no_btn)
        ...
        val tmp_btn: Button = view!!.findViewById(R.id.btnAddCompany_1)
        tmp_btn.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                if (linOtherInsureMore?.childCount!! < maxCountOtherInsure) {
                    createInputCompany(linOtherInsureMore, "", object : View.OnClickListener {
                        override fun onClick(v: View) {
                            linOtherInsureMore?.removeView(v.parent as View?)
                    ...
                    if (CommonFunction.CF_checkAccessibilityTurnOn(activity)) {
                        val tmp_lin: LinearLayout = linOtherInsureMore?.getChildAt(linOtherInsureMore?.childCount!! - 1) as LinearLayout
                        showCustomDialog(resources.getString(R.string.dlg_accessible_open_insure_inputbox), tmp_lin.findViewById(R.id.editText))
                    }
                }
            }
        })
    ...
    private fun setUIOfRealInsureJoin() {
        ...
        val tmp_linInputOtherInsure: View = view!!.findViewById(R.id.linRealInsureInput)
        ...
        switchJoinReal?.CF_getBtnLeft()?.contentDescription = resources.getString(R.string.desc_silson_yes_btn)
        switchJoinReal?.CF_getBtnRight()?.contentDescription = resources.getString(R.string.desc_silson_no_btn)
        ...
        // -- 실손보험사 추가 버튼
        val tmp_btn: Button = view!!.findViewById(R.id.btnAddCompany_2)
        tmp_btn.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View) {
                if (linRealInsureMore?.childCount!! < maxCountOtherInsure) {
                    ...
                    // -- 접근성 On : 보험사명 입력 UI로 이동
                    if (CommonFunction.CF_checkAccessibilityTurnOn(activity)) {
                        val tmp_lin: LinearLayout = linRealInsureMore?.getChildAt(linRealInsureMore?.childCount!! - 1) as LinearLayout
                        showCustomDialog(resources.getString(R.string.dlg_accessible_open_insure_inputbox), tmp_lin.findViewById(R.id.editText))
                    }
                }
            }
    ...
    private fun interceptEditTextOtherFocus() {
        if (CommonFunction.CF_checkAccessibilityTurnOn(activity)) {
            val tmp_lin: LinearLayout = view!!.findViewById(R.id.linOtherInput)
            tmp_lin.descendantFocusability = ViewGroup.FOCUS_BLOCK_DESCENDANTS
            Handler().postDelayed(object : Runnable {
                override fun run() {
                    tmp_lin.descendantFocusability = ViewGroup.FOCUS_BEFORE_DESCENDANTS
                }
            }, 500)
        }
    }
    ...
    private fun createInputCompany(p_parentView: LinearLayout?, p_strText: String, p_onClickRemove: View.OnClickListener) {
        val tmp_linItem: LinearLayout = View.inflate(activity, R.layout.c_input_other_company, null) as LinearLayout
        tmp_linItem.setPadding(0, CommonFunction.CF_convertDipToPixel(activity!!.applicationContext, 5f), 0, 0)
        val tmp_edt: EditText = tmp_linItem.findViewById(R.id.editText)
        tmp_edt.setText(p_strText)
        tmp_edt.privateImeOptions = "defaultInputmode=korean"
        val tmp_btnRemove: ImageButton = tmp_linItem.findViewById(R.id.btnDel)
        tmp_btnRemove.setOnClickListener(p_onClickRemove)
        tmp_btnRemove.visibility = View.VISIBLE
    ...
    private val strOtherInsureCompanyName: String
        get() {
            ...
            if (TextUtils.isEmpty(edtOtherInsure!!.text.toString().trim()) == false) {
                tmp_arrText.add(edtOtherInsure!!.text.toString().trim())
            }
            val tmp_arrMore: ArrayList<String> = otherInsureCompanyNameMoreList
    ...
    private val otherInsureCompanyNameMoreList: ArrayList<String>
        get() {
            ...
            for (i in 0 until linOtherInsureMore!!.childCount) {
                val tmp_linItem: LinearLayout = linOtherInsureMore!!.getChildAt(i) as LinearLayout
                val tmp_edt: EditText = tmp_linItem.findViewById(R.id.editText)
    ...
    private val strRealInsureCompanyName: String
        get() {
            val tmp_arrText: ArrayList<String?> = ArrayList()
            if (TextUtils.isEmpty(edtRealInsure!!.text.toString().trim()) == false) {
                tmp_arrText.add(edtRealInsure!!.text.toString().trim())
            }
    ...
    private fun checkUserInput(): Boolean {
        ...
        // -- 기타
        val temp_edtOther: String = edtOther!!.text.toString().trim()
        val temp_euckr_edtOther: String = FormatUtils.CF_checkUtf8toEucKr(temp_edtOther)
