 ...
 * 0.0.0    NJM_20200602    계약자 != 수익자일때 계좌입력폼 인풋폼으로 고정
 ...
 * </pre>
 */
class IUII90M07_F : IUII90M00_FD(), ObjectHandlerMessage {
    ...
    private var aData: Data_IUII90M00? = null // Activity Data
    ...
    override fun onActivityResult(p_requestCode: Int, p_resultCode: Int, p_data: Intent?) {
        ...
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.onActivityResult()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        ...
        if (p_requestCode == EnvConfig.REQUESTCODE_ACTIVITY_CHOICE_BANK && p_resultCode == Activity.RESULT_OK) {
            ...
            if ((p_data != null) && p_data.hasExtra("code") && p_data.hasExtra("name")) {
                ...
                val bankCode: String? = p_data.extras!!.getString("code")
                val bankName: String? = p_data.extras!!.getString("name")

                // -- 고객이 선택한 금융기관 이름 & 코드 세팅
                // --<> (우체국 & 우체국계좌보유 & 계약관계:수익자) : 스피너로 구성
                if (("071" == bankCode) && (data!!.bankPostAccountArray.isNotEmpty()) && (aData!!.CF_getS_rctr_relt_dvsn() == "42")) {
            ...
            if (p_data != null) {
                // -- Intent 데이터 GET
                val departCode: String? = p_data.extras!!.getString("departCode") // 추천국 코드
                val departName: String? = p_data.extras!!.getString("departName") // 추천국 이름
                val personRemnNo: String? = p_data.extras!!.getString("personRemnNo") // 추천인 번호
                val personName: String? = p_data.extras!!.getString("personName") // 추천인 이름
                ...
                textRecommDepart!!.text = departName
                textRecommDepart!!.contentDescription = "선택창, " + textRecommDepart!!.text + "선택됨, 선택하시려면 두번 누르세요."
                textRecommDepart!!.tag = departCode

                // 고객이 선택한 추천인 이름 & 코드 세팅
                textRecommPerson!!.text = personName
                textRecommPerson!!.contentDescription = "선택창, " + textRecommPerson!!.text + "선택됨, 선택하시려면 두번 누르세요."
                textRecommPerson!!.tag = personRemnNo

                LogPrinter.CF_debug("!---- tmp_departName   : $departName")
                LogPrinter.CF_debug("!---- tmp_departCode   : $departCode")
                LogPrinter.CF_debug("!---- tmp_personName   : $personName")
                LogPrinter.CF_debug("!---- tmp_personRemnNo : $personRemnNo")
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.onCreate()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    override fun onActivityCreated(p_bundle: Bundle?) {
        ...
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.onActivityCreated()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        ...
        if (p_bundle != null && p_bundle.containsKey("data")) {
            ...
            data!!.bankOwner = aData!!.CF_getS_bnfc_nm()
            ...
            // --<> (우체국 & 우체국계좌보유 & 계약관계:수익자) : 스피너로 구성
            if (("071" == mActivity?.CF_getsLastFnisCode()) && (data!!.bankPostAccountArray.isNotEmpty()) && (aData!!.CF_getS_rctr_relt_dvsn() == "42")) {
    ...
    private fun setUIControl() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.setUIControl()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        ...
        // -----------------------------------------------------------------------------------------
        // -- 금융기관(은행명)
        val linBankName: LinearLayout = view!!.findViewById(R.id.labelTextBank)
        val labelBankName: TextView = linBankName.findViewById(R.id.label)
        textBankName = linBankName.findViewById(R.id.text)
        labelBankName.text = resources.getString(R.string.label_bank_name_n)
        ...
        // -- 추천국/추천인
        val linRecommendDepartment: LinearLayout = view!!.findViewById(R.id.labelTextRecommendDepartment) // 추천국
        val linRecommendPerson: LinearLayout = view!!.findViewById(R.id.labelTextRecommendPerson) // 추천인
        val labelRecommendDepartment: TextView = linRecommendDepartment.findViewById(R.id.label)
        val labelRecommendPerson: TextView = linRecommendPerson.findViewById(R.id.label)
        labelRecommendDepartment.text = resources.getString(R.string.label_recomm_depart_name)
        labelRecommendPerson.text = resources.getString(R.string.label_recomm_person_name)
        ...
        textBankName?.contentDescription = "선택창, " + getResources().getString(R.string.hint_bank_company) + ", 선택하시려면 두번 누르세요."
        textBankName?.tag = ""
        ...
        // -- 예금주
        val linBankOwner: LinearLayout = view!!.findViewById(R.id.labelTextName)
        val labelName: TextView = linBankOwner.findViewById(R.id.label)
        labelName.text = getResources().getString(R.string.label_bank_owner)
        textBankOwner = linBankOwner.findViewById(R.id.text)

        // -- 추천국
        textRecommDepart = linRecommendDepartment.findViewById(R.id.text)
        textRecommDepart?.isClickable = true
        textRecommDepart?.hint = getResources().getString(R.string.hint_recomm_depart_select)
        textRecommDepart?.contentDescription = "선택창, " + getResources().getString(R.string.hint_recomm_depart_select) + ", 선택하시려면 두번 누르세요."
        ...
        // -- 추천인
        textRecommPerson = linRecommendPerson.findViewById(R.id.text)
        textRecommPerson?.isClickable = true
        textRecommPerson?.hint = getResources().getString(R.string.hint_recommand_person_name)
        textRecommPerson?.contentDescription = "선택창, " + getResources().getString(R.string.hint_recommand_person_name) + ", 선택하시려면 두번 누르세요."
    ...
    override fun onSaveInstanceState(p_bundle: Bundle) {
        ...
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.onSaveInstanceState()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun restoreData() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.restoreData()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        ...
        // -- 우체국계좌(span)
        val selectedPostAcno: String = data!!.bankPostAccount
        val postAccArray: Array<String>? = data!!.bankPostAccountArray
        try {
            val adapter: ArrayAdapter<String> = ArrayAdapter(mActivity!!.applicationContext, R.layout.layout_label_spinner_list, postAccArray!!)
        ...
        if (!("" == selectedPostAcno)) {
            if (postAccArray != null) {
                for (i in postAccArray.indices) {
                    if ((selectedPostAcno == postAccArray.get(i))) {
                        LogPrinter.CF_debug("!---- 계좌번호 일치(계좌번호) :" + postAccArray.get(i) + "/ index:" + i)
    ...
    private fun saveData() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.saveData()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun checkUserInput(): Boolean {
        ...
        if (textBankName!!.text.toString().trim().isEmpty()) {
            ...
            if (("list" == accountVisibilityDivision)) {
                val acc: String = spn_BankAccount!!.selectedItem.toString()
                if (("" == acc)) {
                    flagOk = false
                }
            } else {
                if (flagOnAccibility) {
                    val edtAccount: EditText = view?.findViewById<View>(R.id.relKeyInput)!!.findViewById(R.id.editText)
                    showCustomDlgAndAccessibilityFocus(resources.getString(R.string.dlg_no_input_bank_account), edtAccount)
                } else {
    ...
    override fun handleMessage(p_message: Message) {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.handleMessage()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        if (mActivity != null && !mActivity!!.isDestroyed) {
            when (p_message.what) {
                HANDLERJOB_CHECK -> {
                    ...
                    try {
                        // -- 결과값 저장
                        httpRes_cehckBankAccount(JSONObject(p_message.obj as String?))
     ...
     * 계좌정보 확인 요청
     */
    private fun httpReq_cehckBankAccount() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.requestCehckBankAccount()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun httpRes_cehckBankAccount(p_jsonObject: JSONObject) {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.setResultOfCheckBankAccount() --계좌정보확인 요청 결과 처리 함수")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        ...
        if (p_jsonObject.has(jsonKey_errorCode)) {
            ...
            when {
                tmp_errorCode == "ERRIUII30M00001" -> {
                    ...
                    CommonFunction.CF_showCustomAlertDilaog(activity, resources.getString(R.string.dlg_error_erriuii30m00001), getResources().getString(R.string.btn_ok))
                }
                tmp_errorCode == "ERRIUII30M00002" -> {
                    LogPrinter.CF_debug("!--<2> (ERRIUII30M00002)예금주 다름")
                    CommonFunction.CF_showCustomAlertDilaog(activity, resources.getString(R.string.dlg_error_erriuii30m00002), getResources().getString(R.string.btn_ok))
                }
                tmp_errorCode == "ERRIUII30M00003" -> {
                    LogPrinter.CF_debug("!--<2> (ERRIUII30M00003)상대은행 처리 지연(전문 에러 EEA683)")
                    CommonFunction.CF_showCustomAlertDilaog(activity, resources.getString(R.string.dlg_error_erriuii30m00003), getResources().getString(R.string.btn_ok))
                }
                    ...
                    customDialog.setOnDismissListener(object : DialogInterface.OnDismissListener {
                        override fun onDismiss(dialog: DialogInterface) {
                            when {
                                ...
                                mActivity?.CF_getAuthDvsn() == AuthDvsn.FINCERT -> { // 금융인증
                                    startIUPC10M00()
                                }
    ...
    private fun startBankListActivity() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startBankListActivity()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    /**
     * IUII33M00_P 추천국/추천인 선택 Activity 호출 함수
     * @since 2019-05-29 최초 등록
     * @param fragmentDvsn  int     0:추천국화면 1:추천인화면
     */
    private fun startIUII33M00(fragmentDvsn: Int) {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startIUII33M00()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")

        val tmp_departCode: String = textRecommDepart!!.getTag() as String // 추천국 코드
    ...
    private fun startIUPC80M00() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startIUPC80M00()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun startIUPC90M00_P() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startIUPC90M00_P()")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
     ...
     * @param authTechCode [Fido2Constant] 간편인증 코드
     */
    private fun startFidoAuth(authTechCode: String) {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startFidoAuth() --간편인증 전자서명 요청")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun startKakaopayAuth() {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startKakaopayAuth() --카카오인증 전자서명 요청")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
    ...
    private fun startIUII33M00EXE(fragmentDvsn: Int) {
        LogPrinter.CF_debug("!-----------------------------------------------------------")
        LogPrinter.CF_debug("!-- IUII90M07_F.startIUII33M00EXE(int fragmentDvsn)")
        LogPrinter.CF_debug("!-----------------------------------------------------------")
